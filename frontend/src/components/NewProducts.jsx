      ...
      <div className="bg-white">
        ...
        <div className="relative aspect-square bg-gray-100 overflow-hidden">
          ...
          {hasDiscount && discountPercent > 0 && (
            <div className="absolute top-2 left-2 bg-[#FF6B35] text-white px-2 py-1 text-xs font-bold rounded-full z-10 shadow-md">
              -{discountPercent}%
            </div>
          )}
        ...
        <div className="p-3">
          ...
          <div className="flex items-baseline gap-2 mb-2">
            {hasDiscount ? (
              <>
                <span className="text-sm font-bold text-black">{Number(salePrice).toLocaleString('vi-VN')}₫</span>
                <span className="text-xs text-gray-500 line-through">{Number(originalPrice).toLocaleString('vi-VN')}₫</span>
              </>
            ) : (
              <span className="text-sm font-bold text-black">{Number(displayPrice).toLocaleString('vi-VN')}₫</span>
            )}
          </div>
...
export default function NewProducts() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const params = {
          sort: 'latest', // Sắp xếp theo sản phẩm mới nhất
        ...
        // Lấy sản phẩm mới nhất (theo createdAt mới nhất)
        const newProducts = (data.items || [])
          .slice(0, 15); // Lấy 15 sản phẩm mới nhất
    ...
    return (
      <section className="bg-white py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center text-gray-500">Đang tải sản phẩm mới...</div>
        </div>
      </section>
    );
  }
